    ...
    async def run(self):
        try:
            if self.question_str:
                ...
                base_path = "https://pr-agent-docs.pr-agent.ai/tools"
                ...
                if isinstance(self.git_provider, GithubProvider) and not get_settings().config.get('disable_checkboxes', False):
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\n(1) Note that each tool can be [triggered automatically](https://pr-agent-docs.pr-agent.ai/usage-guide/automations_and_usage/#github-app-automatic-tools-when-a-new-pr-is-opened) when a new PR is opened, or called manually by [commenting on a PR](https://pr-agent-docs.pr-agent.ai/usage-guide/automations_and_usage/#online-usage)."""
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\nNote that each tool can be [invoked automatically](https://pr-agent-docs.pr-agent.ai/usage-guide/automations_and_usage/) when a new PR is opened, or called manually by [commenting on a PR](https://pr-agent-docs.pr-agent.ai/usage-guide/automations_and_usage/#online-usage)."""
