...
`<pr_url>` is the url of the relevant PR (for example: [#50](https://github.com/rajshree1854/pr-agent/pull/50)).
...
3. **git provider**: The [git_provider](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/configuration.toml#L12) field in a configuration file determines the GIT provider that will be used by PR-Agent. Currently, the following providers are supported:
...
Online usage means invoking PR-Agent tools by [comments](https://github.com/rajshree1854/pr-agent/pull/229#issuecomment-1695021901) on a PR.
...
Any configuration value in [configuration file](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/configuration.toml) file can be similarly edited. Comment `/config` to see the list of available configurations.
...
!!! note "Configurations for PR-Agent"
    PR-Agent for GitHub is an App, hosted by PR Agent. So all the instructions below are relevant for PR-Agent users.
    Same goes for [GitLab webhook](#gitlab-webhook) and [BitBucket App](#bitbucket-app) sections.

#### GitHub app automatic tools when a new PR is opened

The [github_app](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/configuration.toml#L223) section defines GitHub app specific configurations.
...
The JSON structure is equivalent to the yaml data structure defined in [pr_reviewer_prompts.toml](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/pr_reviewer_prompts.toml).
...
Similar to GitHub app, when running PR-Agent from BitBucket App, the default [configuration file](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/configuration.toml) will be initially loaded.
