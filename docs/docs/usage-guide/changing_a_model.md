...
See [here](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/algo/__init__.py) for a list of supported models in PR-Agent.
The default model of PR-Agent is `GPT-5.6` from OpenAI.
To use a different model than the default, you need to edit in the [configuration file](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/configuration.toml#L7) the fields:
...
Also, review the [.secrets_template.toml](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/settings/.secrets_template.toml) file for instructions on how to set keys for other models.
...
If the relevant model doesn't appear [here](https://github.com/rajshree1854/pr-agent/blob/main/pr_agent/algo/__init__.py), you can still use it as a custom model:
