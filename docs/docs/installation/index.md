...
!!! note "Docker Hub namespace migration"
    Releases **`0.34.2` and later** are published under [`pragent/pr-agent`](https://hub.docker.com/r/pragent/pr-agent). Older releases (up to and including `v0.31`) remain at the legacy [`rajshree1854/pr-agent`](https://hub.docker.com/r/rajshree1854/pr-agent) namespace as a frozen archive — no new images are pushed there. The examples on this site reference the new namespace; if you are pinning to a release before `0.34.2`, swap `pragent/pr-agent` for `rajshree1854/pr-agent` in your `image:` / `docker pull` / `uses: docker://` references.
